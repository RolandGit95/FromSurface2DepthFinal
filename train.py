...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Training of Neural Networks, the Barkley Diver"
    )
    ...
    model = nn.DataParallel(STLSTM(hidden_size=hidden_size)).to(
        device
    )  # , device_ids=[0,1])
    ...
    wandb.init(
        ...
        name=f"STLSTM_t32_d_{depth}",
        reinit=True,
        dir="logs/",
    )

    train_dataset, val_dataset = torch.utils.data.random_split(
        BarkleyDataset(X, Y, depths=depths), [train_len, val_len]
    )
    ...
    for epoch in range(epochs):
        for i, batch in tqdm(
            enumerate(train_dataloader), total=len(train_dataset) // batch_size
        ):
        ...
        torch.save(
            model.state_dict(), f"models/STLSTM_t32_d_{depth}_ep{epoch}"
        )
        # for callback in callbacks:
        #    callback.step(val_loss)
        # print(X.shape)
